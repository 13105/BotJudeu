...
class RagialChecker(RagnaChecker):
    ...
    def getLojas(self, tipo_de_loja, item):
        try:
        ...
        for tr in pagina.find_all("tr"):
            ...
            for i, td in enumerate(tr):
                if i != 0:
                    ...
                    #Nenhum item a venda ?
                    if not(len(td.contents) <= 1):
                        #A loja é do tipo de compra(1) ou venda(0) ?
                        loja_tipo = int(td.img["src"][-8:-4] == "_buy")

                        if tipo_de_loja != loja_tipo: break
